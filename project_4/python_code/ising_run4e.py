...
plt.figure(1)
for i in range(len(spins)):
    plt.plot(temp, sampled_energies[i, :], label = 'L{d}'.format(spins[i]))
...
plt.figure(2)
for i in range(len(spins)):
    plt.plot(temp, sampled_magnets[i, :], label = 'L{d}'.format(spins[i]))
...
plt.figure(3)
for i in range(len(spins)):
    plt.plot(temp, sampled_cv[i, :], label = 'L{d}'.format(spins[i]))
plt.ylabel(r'$C_v$')
plt.xlabel('Temperature')
...
plt.figure(4)
for i in range(len(spins)):
    plt.plot(temp, sampled_suscept[i, :], label = 'L{d}'.format(spins[i]))
plt.ylabel(r'$\chi$')
plt.xlabel('Temperature')
