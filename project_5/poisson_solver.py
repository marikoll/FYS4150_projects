...
def poisson_jacobi(g, bc_0y, bc_1y, bc_0x, bc_1x, dx, dy, N_x, N_y, max_iter, f):
    """
    Function to solve our PDE with Jacobis method for boundary problem, i.e
    no flow into walls

    Input:
        g           <float64>       array with previous zeta values
        bc_0y       <float64>       array with BC for y = 0
        bc_1y       <float64>       array with BC for y = 1
        bc_0x       <float64>       array with BC for x = 0
        bc_1x       <float64>       array with BC for x = 1
        ...
        f           <float64>       array with previous psi values

    output:

        f           <float64>       matrix with updated psi values, solution
    """
    dxdx = dx**2
    dydy = dy**2
    dxdxdydy = dxdx*dydy
...
def poisson_jacobi_periodic(g, dx, dy, N_x, N_y, max_iter, f):
        """
        Function to solve our PDE with Jacobis method in periodic domain.

        Input:
            g           <float64>       array with current zeta values
            dx          <float>         stepsize in x-direction
            dy          <float>         stepsize in y-direction
            N_x         <int>           number of points in x-direction
            N_y         <int>           number of points in y-direction
            max_iter    <int>           max number of iterations
            f           <float64>       array with current psi values
